 ...
 */
export function extractSemanticDiffChunks(filePath, fullFileContent, patchText, contextLines = 3) {
    ...
    console.log(`[semanticDiff] Semantic chunks in file: ${semanticChunks.length}`);
    const resultChunks = [];
    semanticChunks.forEach((chunk) => {
        ...
        const contextAfterEnd = Math.min(fileLines.length, Math.max(...allChangedLineNumbers) + contextLines);
        ...
        const contextBefore = fileLines.slice(contextBeforeStart - 1, contextBeforeEnd);
        const contextAfter = fileLines.slice(Math.max(...allChangedLineNumbers), contextAfterEnd);
        // 6. Deterministic ID
        const id = crypto
            .createHash("sha1")
            .update(`${filePath}:${chunk.name}:${startLine}-${endLine}:${intersectingBlocks
            .map((b) => b.blockHeader)
            .join("|")}`)
            .digest("hex");
