 ...
 */
async function findRelatedContext(repoId, searchQuery, topK = 5) {
    try {
 ...
 */
function buildWalkthroughContext(walkthrough) {
    if (!walkthrough || walkthrough.length === 0) {
        return "";
    }
    return ("\n\n### Previously Generated PR Walkthrough:\n" +
        walkthrough
            .map((step, idx) => `${idx + 1}. **${step.file}** - ${step.chunkName} (${step.chunkType})\n   ${step.explanation}`)
            .join("\n\n"));
}
/**
 * Build context from related code chunks found in QDrant
 */
function buildRelatedCodeContext(relatedContext) {
    if (relatedContext.length === 0) {
        return "";
    }
    return ("\n\n### Related Code Context from Repository:\n" +
        relatedContext
            .map((ctx, idx) => `${idx + 1}. **${ctx.file}** - ${ctx.chunkName || "Code Block"} (${ctx.chunkType || "unknown"})\n` +
 ...
 */
function generatePrompt(input, relatedContext) {
 ...
 */
function determineConfidence(relatedContext, hasDirectCode) {
    if (hasDirectCode && relatedContext.length >= 3) {
        const avgScore = relatedContext.reduce((sum, ctx) => sum + (ctx.relevanceScore || 0), 0) /
 ...
 */
export async function answerCodeQuestion(input) {
    console.log(`[ContextualReview] Processing question for repo: ${input.repoId}`);
    // Build search query from question and context
    const searchQuery = buildSearchQuery(input);
    console.log(`[ContextualReview] Search query: ${searchQuery.substring(0, 100)}...`);
    // Find related context from QDrant
    const relatedContext = await findRelatedContext(input.repoId, searchQuery, 5);
    console.log(`[ContextualReview] Found ${relatedContext.length} related code chunks`);
    // Generate AI prompt
