...
export function extractChangedBlocks(filePath, patchText) {
    ...
    console.log(`[extractChangedBlocks] File: ${filePath}, patch lines: ${lines.length}`);
        ...
        if (line.startsWith("@@")) {
            ...
            if (blockMatch) {
                ...
                if (blockLines.length > 0) {
                    const changedLines = blockLines.filter((l) => l.type === "insert" || l.type === "delete");
    ...
    console.log(`[extractChangedBlocks] Found ${blockCount} blocks with changes for ${filePath}`);
    return results;
}
